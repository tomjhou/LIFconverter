...
class LifFile:
    ...
    def __init__(self, filename):
        ...
        if len(self.image_list) != len(self.offsets) and not truncated:
            raise ValueError(f"Number of images {len(self.image_list)} is not equal to number of "
                             f"offsets {len(self.offsets)}, and file does not appear "
                             "truncated. Something has gone wrong.")
        else:
